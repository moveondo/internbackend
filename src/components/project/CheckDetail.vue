  ...
  <div class="table">
    <div class="crumbs">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item>
          <i class="el-icon-document"></i> {{$t('checkdetail.icon')}} </el-breadcrumb-item>
        <el-breadcrumb-item> {{$t('checkdetail.icon1')}}</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="message">
    ...
    <el-table :data="tableData" style="width: 100%">
      <el-table-column prop="Dataitem" :label="$t('checkdetail.item')" width="180"></el-table-column>
      <el-table-column prop="ktpPhotoUrl" :label="$t('checkdetail.img1')" width="180" header-align="center">
      ...
      <el-table-column prop="ktpSelfPhotoUrl" :label="$t('checkdetail.img2')" width="180">
        <template slot-scope="scope">
      ...
      <el-table-column :label="$t('checkdetail.check')">
        <template slot-scope="scope">
          <!-- <div>{{ scope.row.BCFstatus}}</div> -->
          <div v-if="scope.$index==1 && scope.row.BCFstatus=='-1'">
            <el-button size="small" type="primary" @click="Verify(scope.$index, scope.row)">{{$t('checkdetail.pass')}}</el-button>
            <el-button size="small" type="warning" @click="Refuse(scope.$index, scope.row)">{{$t('checkdetail.back')}} </el-button>
          </div>
          <div v-else-if="scope.$index==1 && scope.row.BCFstatus=='0'">
            <el-button size="small" type="primary" @click="Verify(scope.$index, scope.row)">{{$t('checkdetail.pass')}}</el-button>
            <el-button size="small" type="warning" disabled @click="Refuse(scope.$index, scope.row)"> {{$t('checkdetail.back')}}</el-button>
          </div>
          <div v-else-if="scope.$index==1 && scope.row.BCFstatus=='1'">
            <el-button size="small" type="primary" disabled @click="Verify(scope.$index, scope.row)">{{$t('checkdetail.pass')}}</el-button>
            <el-button size="small" type="warning" disabled @click="Refuse(scope.$index, scope.row)"> {{$t('checkdetail.back')}} </el-button>
          </div>
      ...
      <el-table-column prop="status" :label="$t('checkdetail.status')" width="180"></el-table-column>
    </el-table>

    <div class="ktpInfo">
      <div class="Order">
        <div class="text" @click="ClickImglive()">{{$t('checkdetail.foto')}}:
          <img v-bind:src="liveDetectPhotoUrl" />
        </div>
      </div>
      <div class="Order">
        <div class="text">{{$t('checkdetail.result')}}:{{detectFlag}} </div>
      </div>
    </div>

    <el-form :model="ruleForm" ref="ruleForm" label-width="50px" class="select" style="margin-top:60px;">
      <el-form-item :label="$t('checkdetail.choice')" prop="">
        <el-select v-model="ruleForm.value1" :placeholder="$t('checkdetail.choice')">
          <el-option v-for="item1 in answerPeople" :key="item1.value" :label="item1.option" :value="item1.value"></el-option>
        </el-select>
        <el-select v-model="ruleForm.value2" :placeholder="$t('checkdetail.choice')">
          <el-option v-for="item in answerResult" :key="item.value" :label="item.option" :value="item.value"></el-option>
        </el-select>
        <el-button type="primary" @click="submitForm('ruleForm')"> {{$t('checkdetail.commit')}} </el-button>
        <el-tag>
          <router-link target="_blank" :to="{ path: '/internalrecord',query:{userId:this.userId,userName:this.userName}}">{{$t('checkdetail.inner')}} </router-link>
        </el-tag>
        <el-tag>
          <router-link target="_blank" :to="{ path: '/auditrecord',query:{userId:this.userId}}">{{$t('checkdetail.record')}}</router-link>
        </el-tag>
      </el-form-item>
    </el-form>
    ...
    <div class="first">
      ...
      {{$t('checkdetail.status')}}:
      <span> {{listingStatus}}</span>
    </div>
    <hr/>
    <div class="check">
      <div v-if=" active===true ">
        <el-button type="primary" id="qq" @click="First('pass')"> {{$t('checkdetail.checkf')}}</el-button>
        <el-button type="warning" @click="dialogFormVisible = true"> {{$t('checkdetail.ref1')}}</el-button>
        <!-- @click="First('refuse')" -->
      </div>
      <div v-else>
    ...
    <el-dialog :title="$t('checkdetail.refuse')" :visible.sync="dialogFormVisible" style="margin-left:50px;">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogFormVisible = false"> {{$t('checkdetail.ref')}}</el-button>
        <el-button type="primary" @click="First('refuse')" v-bind:disabled="isActive"> {{$t('checkdetail.commit')}} </el-button>
      </div>
    </el-dialog>
    ...
    methods: {
      ...
      submitData(type) {
        ...
        if (type == 'phone') {
          ...
          operate = 3;
          auditContent = this.$t('checkdetail.checkf');
        ...
        self.$axios.post(self.submitinfo, {
          ...
          if (res.data.result == 0) {
            ...
            self.$message.success(this.$t('checkdetail.tl'));
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            if (self.ruleForm.value1 == '' || self.ruleForm.value1 == null) {
              self.$message.error(this.$t("checkdetail.lian"));
              return false;
            }
            if (self.ruleForm.value2 == '' || self.ruleForm.value2 == null) {
              self.$message.error(this.$t("checkdetail.lian2"));
              return false;
            }
      ...
      First(msg) {
        ...
        if (msg == "pass") {
          ...
          self.$axios.post(self.FirstListing, {
            ...
            if (res.data.result == 0) {
              this.active = false;
              this.message = this.$t('checkdetail.vt');
              self.message = self.listingStatus;
              this.$message.success(this.$t('checkdetail.vt'));
            } else {
          ...
          if (self.checkList.length == 0) {
            this.$message.error(this.$t('checkdetail.sp'));
            return false;
          }
          self.isActive = true;
          ...
          self.$axios.post(self.FirstListing, {
            ...
            if (res.data.result == 0) {
              this.active = false;
              this.message = this.$t('checkdetail.pa');
              this.$message.success(this.$t('checkdetail.pa'));
              self.isActive = false;
            } else {
              this.$message.error(res.data.resultMessage);
              self.isActive = false;
      ...
      queryLiveDetect(userId) {
        ...
        self.$axios.post(self.url6, {
          ...
          if (res.data.result == 0) {
            ...
            let livePhotoUrl = res.data.content.liveDetectPhotoUrl;
            if (detectFlagR === true) {
              self.detectFlag = this.$t('checkdetail.success');
              self.liveDetectPhotoUrl = res.data.content.liveDetectPhotoUrl;
            } else if (livePhotoUrl !== "" && detectFlagR === false) {
              self.detectFlag = this.$t('checkdetail.failure');
            } else {
              self.detectFlag = this.$t('checkdetail.uncertified');
            }
        ...
        if (index == 0) {
          ...
          self.$axios.post(self.VerifyKTPurl, {
            ...
            if (res.data.result == 0) {
              ...
              this.$message.success(this.$t('checkdetail.tl'));
              this.Reload();
            } else if (res.data.result == '-99' || res.data.result == '-999') {
              self.$message.error(res.data.resultMessage);
            } else {
      ...
      Refuse(index, row) {
        ...
        if (index == 0) {
          ...
          self.$axios.post(self.VerifyKTPurl, {
            ...
            if (res.data.result == 0) {
              this.$message.success(this.$t('checkdetail.ref1'));
            } else if (res.data.result == '-99' || res.data.result == '-999') {
              self.$message.error(res.data.resultMessage);
            } else {
              self.$message.error(res.data.resultMessage);
