  ...
  <div class="table">
    <div class="crumbs">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item>
          <i class="el-icon-document"></i> {{$t('checkdetailtwo.icon')}} </el-breadcrumb-item>
        <el-breadcrumb-item> {{$t('checkdetailtwo.icon1')}} </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="message">
      <table class="table table-striped table-bordered">
        <tbody>
          <tr :key="index" v-for="(item, index) of UserMessage">
            <td>{{$t('checkdetailtwo.base')}} :</td>
            <td>
              <router-link target="_blank" :to="{ path: '/checkpeople',query:{userId:item.userId}}">{{item.ktpName}}</router-link> ,</td>
            <td>listingid:{{item.id}}, </td>
    ...
    <el-table :data="tableData" style="width: 100%">
      <el-table-column prop="Dataitem" :label="$t('checkdetailtwo.item')" width="180"></el-table-column>
      <el-table-column prop="ktpPhotoUrl" :label="$t('checkdetailtwo.img1')" width="180" header-align="center">
        <template slot-scope="scope">
          <span v-if="scope.row.ktpPhotoUrl" @click="ClickImg(scope.row.ktpPhotoUrl,scope.row)">
            <img :src="scope.row.ktpPhotoUrl" width="90" height="90" />
          </span>
          <span v-else> </span>
        </template>
      </el-table-column>
      <el-table-column prop="ktpSelfPhotoUrl" :label="$t('checkdetailtwo.img2')" width="180">
        <template slot-scope="scope">
          <span v-if="scope.row.ktpSelfPhotoUrl" @click="ClickImg(scope.row.ktpSelfPhotoUrl,scope.row)">
            <img :src="scope.row.ktpSelfPhotoUrl" width="90" height="90" />
          </span>
          <span v-else> </span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('checkdetailtwo.check')">
        <template slot-scope="scope">
          <!-- <div>{{ scope.row.BCFstatus}}</div> -->
          <div v-if="scope.$index==1 && scope.row.BCFstatus=='-1'">
            <el-button size="small" type="primary" @click="Verify(scope.$index, scope.row)"> {{$t('checkdetailtwo.pass')}}</el-button>
            <el-button size="small" type="warning" @click="Refuse(scope.$index, scope.row)"> {{$t('checkdetailtwo.back')}} </el-button>
          </div>
          <div v-else-if="scope.$index==1 && scope.row.BCFstatus=='0'">
            <el-button size="small" type="primary" @click="Verify(scope.$index, scope.row)"> {{$t('checkdetailtwo.pass')}}</el-button>
            <el-button size="small" type="warning" disabled @click="Refuse(scope.$index, scope.row)"> {{$t('checkdetailtwo.back')}}</el-button>
          </div>
          <div v-else-if="scope.$index==1 && scope.row.BCFstatus=='1'">
            <el-button size="small" type="primary" disabled @click="Verify(scope.$index, scope.row)"> {{$t('checkdetailtwo.pass')}} </el-button>
            <el-button size="small" type="warning" disabled @click="Refuse(scope.$index, scope.row)"> {{$t('checkdetailtwo.back')}}</el-button>
          </div>
      ...
      <el-table-column prop="status" :label="$t('checkdetailtwo.status')" width="180"></el-table-column>
    </el-table>


    <div class="ktpInfo">
      <div class="Order">
        <div class="text" @click="ClickImglive()">{{$t('checkdetailtwo.foto')}}:
          <img v-bind:src="liveDetectPhotoUrl" />
        </div>
      </div>
      <div class="Order">
        <div class="text">{{$t('checkdetailtwo.result')}}:{{detectFlag}} </div>
      </div>
    </div>

    <el-form :model="ruleForm" ref="ruleForm" label-width="50px" class="select" style="margin-top:60px;">
      <el-form-item :label="$t('checkdetailtwo.choice')" prop="">
        <el-select v-model="ruleForm.value1" :placeholder="$t('checkdetailtwo.choice')">
          <el-option v-for="item1 in answerPeople" :key="item1.value" :label="item1.option" :value="item1.value"></el-option>
        </el-select>
        <el-select v-model="ruleForm.value2" :placeholder="$t('checkdetailtwo.choice')">
          <el-option v-for="item in answerResult" :key="item.value" :label="item.option" :value="item.value"></el-option>
        </el-select>
        <el-button type="primary" @click="submitForm('ruleForm')">{{$t('checkdetailtwo.commit')}}</el-button>
        <el-tag>
          <router-link target="_blank" :to="{ path: '/internalrecord',query:{userId:this.userId}}">{{$t('checkdetailtwo.inner')}}</router-link>
        </el-tag>
        <el-tag>
          <router-link target="_blank" :to="{ path: '/auditrecord',query:{userId:this.userId}}">{{$t('checkdetailtwo.record')}}</router-link>
        </el-tag>
      </el-form-item>
    </el-form>
    ...
    <div class="first">
      ...
      <span>{{$t('checkdetailtwo.status')}}:{{listingStatus}}</span>
    </div>
    <hr/>
    <div class="check">
      <div v-if=" active===true ">
        <el-button type="primary" id="qq" @click="First('pass')">{{$t('checkdetailtwo.checkt')}}</el-button>
        <el-button type="warning" @click="dialogFormVisible = true">{{$t('checkdetailtwo.ref1')}}</el-button>
      </div>
      <div v-else>
    ...
    <el-dialog :title="$t('checkdetailtwo.refuse')" :visible.sync="dialogFormVisible" style="margin-left:50px;">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogFormVisible = false">{{$t('checkdetailtwo.back')}}</el-button>
        <el-button type="primary" @click="First('refuse')">{{$t('checkdetailtwo.commit')}}</el-button>
      </div>
    </el-dialog>
...
<script>
  ...
  export default {
    ...
    watch: {
      locale() {
        this.queryPersonData(this.$route.query.listingId);
        this.queryInfo();
      }
    },
    methods: {
      ...
      submitData(type) {
        ...
        if (type == 'phone') {
          ...
          operate = 4;
          auditContent = this.$t('checkdetailtwo.checkt');
        ...
        self.$axios.post(self.submitinfo, {
          "userId": userId,
          "listingId": listingId,
          "auditName": auditName,
          "operate": operate,
          ...
          if (res.data.result == 0) {
            self.$message.success(this.$t('checkdetailtwo.tl'));
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            if (self.ruleForm.value1 == '' || self.ruleForm.value1 == null) {
              self.$message.error(this.$t('checkdetailtwo.lian'));
              return false;
            }
            if (self.ruleForm.value2 == '' || self.ruleForm.value2 == null) {
              self.$message.error(this.$t('checkdetailtwo.lian2'));
              return false;
            }
      ...
      ClickImg(imgURL, row) {
        ...
        let KTPNO = row.ktpNo;
        let img = imgURL;
        let imgSRC = "//indonesia-object.oss-ap-southeast-5.aliyuncs.com/ktpDeal/imgdeal.html?KTPname=" + KTPname +
          "&KTPNO=" + KTPNO + "&imgurl=" + img;
      ...
      First(msg) {
        ...
        if (msg == "pass") {
          ...
          self.$axios.post(self.FirstListing, {
            "userId": userId,
            "listingId": listingId,
            "auditStatus": 2
          }).then((res) => {
            if (res.data.result == 0) {
              this.active = false;
              this.message = this.$t('checkdetailtwo.vt');
              self.message = self.listingStatus;
              this.$message.success(this.$t('checkdetailtwo.vt'));
            } else {
          ...
          if (self.checkList.length == 0) {
            this.$message.error(this.$t('checkdetailtwo.sp'));
            return false;
          }
          ...
          self.$axios.post(self.FirstListing, {
            ...
            if (res.data.result == 0) {
              this.active = false;
              this.message = this.$t('checkdetailtwo.pa');
              this.$message.success(this.$t('checkdetailtwo.pa'));
            } else if (res.data.result == '-99' || res.data.result == '-999') {
              self.$message.error(res.data.resultMessage);
            } else {
              self.$message.error(res.data.resultMessage);
      ...
      queryPersonData(listingId) {
        ...
        var statusName;
        self.$axios.post(self.personurl, {
          "listingId": listingId
        }).then((res) => {
          ...
          for (let i = 0; i < Length; i++) {
            ...
            self.tableData = [{ //重新加载使用
              ...
              Dataitem: self.$t('checkdetail.bankCard'),
              ktpPhotoUrl: '',
              ktpSelfPhotoUrl: ''
            }]
      ...
      queryLiveDetect(userId) {
        ...
        self.$axios.post(self.url6, {
          ...
          if (res.data.result == 0) {
            ...
            let liveDetectPhotoUrl = res.data.content.liveDetectPhotoUrl;
            if (detectFlagR === true) {
              self.detectFlag = this.$t('checkdetailtwo.success');
              self.liveDetectPhotoUrl = res.data.content.liveDetectPhotoUrl;
            } else if (liveDetectPhotoUrl !== '' && detectFlagR === false) {
              self.detectFlag = this.$t('checkdetailtwo.failure');
            } else {
              self.detectFlag = this.$t('checkdetailtwo.uncertified');
            }
      ...
      CheckCode(content, type) {
        ...
        if (content == "listing") {
          if (type == 10 || type == 20) {
            statusName = this.$t('checkdetail.pendingStatus1');
          } else if (type == 30) {
            statusName = this.$t('checkdetail.pendingStatus2');
          }
          return statusName;
        }
        ...
        if (content == "bankORkpt") {
          if (type == 0) {
            statusName = this.$t('checkdetail.pass')
          } else if (type == 1) {
            statusName = this.$t('checkdetail.pendingAudit')
          } else if (type == -1) {
            statusName = this.$t('checkdetail.back')
          }
          return statusName;
        }
      ...
      Verify(index, row) {
        ...
        if (index == 0) {
          ...
          self.$axios.post(self.VerifyKTPurl, {
            "userId": userId,
            "ktpNo": ktpNo,
            "ktpName": ktpName,
            "auditStatus": 0
            ...
            if (res.data.result == 0) {
              ...
              this.$message.success(this.$t('checkdetailtwo.tl'));
              this.Reload();
            } else if (res.data.result == '-99' || res.data.result == '-999') {
              self.$message.error(res.data.resultMessage);
            } else {
        ...
        if (index == 1) {
          ...
          self.$axios.post(self.VerifyBankurl, {
            "id": id,
            "userId": userId,
            "bankCardNo": bankCardNo,
            "bankCode": bankCode,
      ...
      Refuse(index, row) {
        ...
        if (index == 0) {
          ...
          self.$axios.post(self.VerifyKTPurl, {
            ...
            if (res.data.result == 0) {
              this.$message.success(this.$t('checkdetailtwo.ref1'));
            } else if (res.data.result == '-99' || res.data.result == '-999') {
              self.$message.error(res.data.resultMessage);
            } else {
              self.$message.error(res.data.resultMessage);
